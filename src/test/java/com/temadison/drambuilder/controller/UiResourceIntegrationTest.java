...
class UiResourceIntegrationTest {
    ...
    void servesDashboardAndStaticModules() throws Exception {
        ...
        mockMvc.perform(get("/data.html"))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("Data Load Status")))
                .andExpect(content().string(containsString("Manual Data Tools")))
                .andExpect(content().string(containsString("Refresh Latest Data")))
                .andExpect(content().string(containsString("Issuer Refresh")))
                .andExpect(content().string(containsString("Roundhill NAV, holdings, prices, FX, snapshot")))
                ...
                .andExpect(content().string(containsString("Local File Import")))
                .andExpect(content().string(containsString("refresh-roundhill-ingestion-button")))
        ...
        mockMvc.perform(get("/js/app.js"))
                ...
                .andExpect(content().string(containsString("runRoundhillIngestion")))
                .andExpect(content().string(containsString("bindClick('refresh-button', refreshLatestData)")))
                .andExpect(content().string(containsString("Refreshing latest Roundhill issuer data")))
                .andExpect(content().string(containsString("ingestionSummary")))
                .andExpect(content().string(containsString("setBusy")))
                .andExpect(content().string(containsString("bindClick")))
                .andExpect(content().string(containsString("importSummary")))
                .andExpect(content().string(containsString("Snapshot created")))
        ...
        mockMvc.perform(get("/js/sampleData.js"))
                ...
                .andExpect(content().string(containsString("snapshot_holding")))
    ...
    void apiIndexListsAvailableDramEndpoints() throws Exception {
        mockMvc.perform(get("/api/dram"))
                ...
                .andExpect(jsonPath("$.endpoints", hasItem("POST /api/market-data/ingest/roundhill")))
